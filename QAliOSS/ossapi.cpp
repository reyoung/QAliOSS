...
Response QAliOSS::OSSApi::putBucket(const QString &bucket_name, const QString &acl, QMap<QString, QString> headers) const
{
    if (!acl.isEmpty()){
        headers.insert("x-oss-acl",acl);
    }
    return this->doBucketOperation("PUT",bucket_name,headers);
}
